...
<div class="wrap" id="was-disparo-app">
    ...
    <div id="disparo-phone-context" class="notice notice-warning" style="display:none;padding:10px 14px;"></div>
    ...
    <div class="was-dispar-grid">
        <section class="was-dispar-card">
            ...
            <p><a href="<?php echo esc_url(WAS_PLUGIN_URL . 'assets/examples/disparo-exemplo.csv'); ?>" download>Baixar CSV de exemplo</a></p>
        </section>

        <section class="was-dispar-card">
            <h2>Prévia do template</h2>
            <p class="description">A prévia usa exemplos para as variáveis.</p>
            <div id="disparo-template-preview" class="was-dispar-preview">Selecione um template.</div>
        </section>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const pageParams = new URLSearchParams(window.location.search), selectedPhoneId = pageParams.get('was_phone_number_id') || '', selectedTenantId = pageParams.get('was_tenant_id') || '';
    const api = async (path, method='GET', body=null) => { if(method==='GET' && (selectedPhoneId||selectedTenantId)){const q=[];if(selectedPhoneId)q.push('phone_number_id='+encodeURIComponent(selectedPhoneId));if(selectedTenantId)q.push('tenant_id='+encodeURIComponent(selectedTenantId));path += (path.includes('?')?'&':'?')+q.join('&');} const o={method,headers:{'X-WP-Nonce':wasApp.nonce,'Accept':'application/json'}}; if(body){body.phone_number_id=selectedPhoneId;body.tenant_id=selectedTenantId;o.headers['Content-Type']='application/json';o.body=JSON.stringify(body)} const r=await fetch(wasApp.restUrl+path,o); const d=await r.json(); if(!r.ok) throw new Error(d.message||d.error||'Erro na API'); return d; };
    ...
    const renderPreview = () => { const box=document.getElementById('disparo-template-preview'); if(!selected){box.textContent='Selecione um template.';return;} let body=selected.body_text||'', header=selected.header_text||''; let map={}; try{map=JSON.parse(selected.variable_map||'{}')}catch(e){} Object.entries(map).forEach(([pos,name])=>{const example=String(name).replace(/_/g,' '); body=body.replace(new RegExp('\\{\\{\\s*'+pos+'\\s*\\}\\}','g'),example).replace(new RegExp('\\{\\{\\s*'+name+'\\s*\\}\\}','g'),example); header=header.replace(new RegExp('\\{\\{\\s*'+pos+'\\s*\\}\\}','g'),example).replace(new RegExp('\\{\\{\\s*'+name+'\\s*\\}\\}','g'),example);}); box.innerHTML=`${header?`<div class="was-dispar-preview-header">${esc(header)}</div>`:''}<div class="was-dispar-preview-body">${esc(body).replace(/\n/g,'<br>')}</div>${selected.footer_text?`<div class="was-dispar-preview-footer">${esc(selected.footer_text)}</div>`:''}<small>${esc(selected.name)} · ${esc(selected.language||'')}</small>`; };
    const updateHelp = () => { selected=templates.find(t=>String(t.id)===templateSelect.value); const vars=selected?varsOf(selected):[]; document.getElementById('dispar-variable-help').innerHTML=selected ? (vars.length?'Variáveis obrigatórias: <strong>'+vars.map(esc).join(', ')+'</strong>.':'Este template não possui variáveis.'):'Selecione um template para ver as variáveis.'; renderPreview(); };
    const phoneContext=document.getElementById('disparo-phone-context'); phoneContext.style.display='block'; phoneContext.textContent=selectedPhoneId?'Número associado: '+(pageParams.get('was_phone_display')||selectedPhoneId):'Selecione um número WhatsApp para iniciar um Disparo. Abra esta tela pelo botão Disparo dentro do número.';
    ...
    templateSelect.addEventListener('change',updateHelp); csvInput.addEventListener('change',()=>previewBtn.disabled=!csvInput.files.length || !selectedPhoneId);
